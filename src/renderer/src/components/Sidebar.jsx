...
const Sidebar = () => {
  ...
  return (
    ...
    >
      <div
        className={`p-4 border-b border-gray-200 dark:border-[#27272a] flex flex-col ${!isExpanded && 'items-center'}`}
      >
        <div
          className={`flex items-center ${isExpanded ? 'justify-between' : 'justify-center flex-col gap-4'} mb-4 w-full`}
        >
          {isExpanded && (
            <motion.h2
              initial={{ opacity: 0 }}
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {pendingRequests.length > 0 ? (
                        pendingRequests.map((req) => (
                          <div
                            key={req.id}
                            className="p-3 flex items-center justify-between border-b border-gray-100 dark:border-[#27272a] hover:bg-gray-50 dark:hover:bg-[#27272a] transition-colors"
                          >
                            <div className="flex items-center gap-2">
                              <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-bold uppercase overflow-hidden shrink-0">
                                {req.avatar ? (
                                  <img
                                    src={req.avatar}
                                    alt="Profil"
                                  ...
                                  <span>{req.name ? req.name.charAt(0) : 'U'}</span>
                                )}
                              </div>
                            ...
                            <div className="flex gap-1">
                              <button
                                disabled={respondLoading == 'accepted'}
                                onClick={() => handleRequestResponse(req.id, 'accepted')}
                                className="p-1.5 bg-green-100 text-green-600 dark:bg-green-500/20 dark:text-green-400 rounded-md hover:bg-green-200 dark:hover:bg-green-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {respondLoading == 'accepted' ? (
                              ...
                              <button
                                disabled={respondLoading == 'rejected'}
                                onClick={() => handleRequestResponse(req.id, 'rejected')}
                                className="p-1.5 bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400 rounded-md hover:bg-red-200 dark:hover:bg-red-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {respondLoading == 'rejected' ? (
                                  <Loader2 size={14} className="animate-spin" />
                                ) : (
                                  <X size={14} />
                                )}
      ...
      <div
        className={`flex-1 overflow-y-auto p-3 space-y-1 ${!isExpanded && 'flex flex-col items-center overflow-x-hidden'}`}
      >
        {isLoadingFriends ? (
          [1, 2, 3, 4, 5].map((item) => (
            <div
              key={item}
              className={`flex items-center ${isExpanded ? 'gap-3 p-3' : 'justify-center p-2 mb-1'} w-full rounded-xl`}
            >
              ...
              >
                ...
                <div className="relative flex-shrink-0">
                  <div
                    ...
                    title={friend?.username || friend?.displayName}
                  >
                    {friend?.avatar ? (
                      <img
                        src={friend.avatar}
                        alt={friend?.username || 'Profil'}
                        className="w-full h-full object-cover"
                      />
                    ) : (
                      <span>{friend?.username?.charAt(0) || 'E'}</span>
                    )}
                  </div>
                  {friend.status === 'online' && (
                    <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-500 border-2 border-white dark:border-[#09090b] group-hover:border-gray-100 dark:group-hover:border-[#18181b] rounded-full z-10 transition-colors"></span>
                  )}
                </div>
                  ...
                  <>
                    <div className="flex-1 whitespace-nowrap overflow-hidden">
                      ...
                      <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                        ...
                        {friend.status === 'online'
                          ? `${friend.focusStatus?.activityName ? friend.focusStatus.activityName : 'Boşta'}`
                          : 'Molada/Çevrimdışı'}
                      </p>
                    </div>
      ...
      <div
        className={`p-4 border-t border-gray-200 dark:border-[#27272a] flex items-center bg-white dark:bg-[#09090b] ${isExpanded ? 'justify-between' : 'flex-col gap-4 justify-center'}`}
      >
        <div className={`flex items-center ${isExpanded ? 'gap-3' : 'justify-center'}`}>
          {/* === KULLANICI AVATAR VE ONLİNE NOKTASI === */}
          <div className="relative flex-shrink-0">
            <div
              className="w-10 h-10 rounded-full bg-gray-800 dark:bg-gray-200 flex items-center justify-center text-white dark:text-gray-800 font-bold uppercase overflow-hidden"
            ...
            >
              {user?.avatar ? (
                ...
                <span>{user?.username?.charAt(0) || 'E'}</span>
              )}
            </div>
            {/* Kullanıcı kendi profilinde her zaman online'dır (yeşil yanar) */}
            <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-500 border-2 border-white dark:border-[#09090b] rounded-full z-10"></span>
          </div>
        ...
        <div className={`flex items-center ${isExpanded ? 'gap-1' : 'flex-col gap-2'}`}>
          ...
          {isExpanded && (
            <button
              ...
              className="p-2 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-lg hover:bg-gray-100 dark:hover:bg-[#18181b] transition-colors"
              title="Ayarlar"
            >
              <Settings size={18} />
            </button>
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                    <span className="font-bold text-gray-700 dark:text-gray-300">
                      {deleteFriendInfo.username}
                    </span>{' '}
                    adlı kullanıcıyı arkadaşlıktan çıkarmak istediğinize emin misiniz?
                  </p>
                  <div className="flex gap-3">
                    <button
                      type="button"
                      disabled={respondLoading === 'remove'}
                      onClick={() => setDeleteFriendInfo(null)}
                      className="flex-1 py-2 text-sm font-bold bg-gray-100 dark:bg-[#27272a] text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-[#3f3f46] rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Vazgeç
                    </button>
                    <button
                      ...
                      className="flex-1 flex items-center justify-center py-2 text-sm font-bold bg-red-500 hover:bg-red-600 text-white rounded-xl shadow-lg shadow-red-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {respondLoading === 'remove' ? (
